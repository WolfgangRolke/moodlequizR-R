subsetting <- function() {
   category <- "Introduction to R   / Subsetting"
   quizname <- "problem - " 
   nyears <- sample(5:15, 1)
   x <- length(Years[Years>=nyears])
   qtxt <- paste0("In this quiz we will use the data set wrinccensus, 
        already part of Resma3.
        <p>1) There are ", nm(x), " employees with ", nyears, " or more years of service.")
   atxt <- paste0("attach(wrinccensus)<p>
   isubset conditions: <p>1) Years more or equal to ", nyears)
