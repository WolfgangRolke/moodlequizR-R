quiz15.mean.median.sd <- function(which = c("mean", "median", "sd")) {
    ...
    qSD <- ifelse( "sd" %in% which,
        paste0("<p>the standard deviation is {2:NM:=", out[3], "~%80%", out[3], ":", out[3]/1000, "}<p>"), "")
    ...
    aSD <- ifelse( "sd" %in% which,
        paste0("<p>standard deviation = ", out[3], "<br>R Command: sd(x)<p>"), "")
        
    qtxt <- paste0("<h5> Here is some data:<hr>", datatbl, "<hr>", 
            qMean, qMedian, qSD, "<h5>") 
    atxt <- paste0("<h5>Correct answers", aMean, aMedian, aSD, "</h5>")
