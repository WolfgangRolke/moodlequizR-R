hp.mean <- function(type, whichstory, doGraph=FALSE, doAssumptions=FALSE) {
   ...
   if(whichstory == 3) {
         mu <- runif(1,20,200)                
         par0 <- round(runif(1,mu-5,mu+5))                    
         x <- round(abs(rnorm(n,mu,mu)), 1)
         x[x<0] <- 0
         est <- round(c(mean(x), sd(x)), 2)   
   }   
   if(whichstory == 4) {
         mu <- runif(1, 5000, 15000)             
         par0 <- round(runif(1,mu-1000,mu+1000), -1)                    
         x <- round(abs(rnorm(n, mu, mu)), -2)
         x[x<0] <- 0
         est <- round(c(mean(x), sd(x)), -1)
    }   
    if(whichstory == 5) {
         mu <- runif(1,2.5,3.0)             
         par0 <- round(runif(1,mu-0.1000,mu+0.1), 1)                    
         x <- round(abs(rnorm(n,mu,mu)), 1)
         x[x>4] <- 4
         est <- round(c(mean(x), sd(x)), 2)
    } 
   ...
   if(whichstory == 3) {
      if(type == "Data") 
         qtxt <- paste0("A store has just done a major add compaign on TV, and they want 
            to know what effect it has had. They randomly collect some sales receipts. 
            The data is below. Before the campaign the mean amount of a sale was $",par0,". 
            Test at the ", 100*alpha,"% level whether the true mean amount of money is now ",
             alt.txt," $", par0) 
      else
         qtxt <- paste0("A store has just done a major add compaign on TV, and they want to 
           know what effect it has had. They randomly collect ", n, " sales receipts. They 
           find a mean amount of ", est[1], " with a standard deviation of ", est[2],
           ". Before the campaign the mean amount of a sale was $",par0,". Test at the ", 
           100*alpha,"% level whether the true mean amount of money is now ", alt.txt," $",par0) 
                    
   }
   if(whichstory == 4) {
      if(type == "Data") 
         qtxt <- paste0(" An insurance company is interested in the amount of money they pay 
            on average on insurance claims. Some years ago a study showed that the mean payout 
            was $", par0, " They randomly select a number of policies and find the payouts. 
            The data is below. Test at the ", 100*alpha,"% level whether the mean payout is 
            now ", alt.txt," $", par0 )      
         else
            qtxt <- paste0(" An insurance company is interested in the amount of money they 
              pay on average on insurance claims. Some years ago a study showed that the 
              mean payout was $", par0, " They randomly select ",n," policies and find a 
              mean payout of ", est[1], " with a standard deviation of ", est[2],
              ". Test at the ", 100*alpha,"% level whether the mean payout is now ",
               alt.txt," $", par0 )      
   }                            
   if(whichstory == 5) {
      if(type == "Data") 
         qtxt <- paste0("The chancellor of some university reads in the newsaper that the 
            mean GPA at some other university is ",par0,". He wants to know how that compares 
            to the mean GPA at his university, so he randomly collects the GPAs of some 
            students. The data is below. Test at the ", 100*alpha,"% level whether the mean 
            GPA of his students is ", alt.txt," ",par0)
      else
         qtxt <- paste0("The chancellor of some university reads in the newsaper that the 
         mean GPA at some other university is ", par0,". He wants to know how that compares 
         to the mean GPA at his university, so he randomly collects the GPAs of ", n, 
         " students and finds a mean GPA of ", est[1], " with a standard deviation of ",
         est[2], ". Test at the ", 100*alpha,"% level whether the mean
        GPA of his students is ", alt.txt," ", par0)
   }
