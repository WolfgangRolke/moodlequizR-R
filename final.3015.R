final.3015 <- function(problem) {
   category <- paste0(" Exams / Final Exam / Problem = ", problem)
   quizname <- "problem - " 
   htxt <- paste("  ")   
   ...
   if(problem==1) {
        ...
        qtxt <- paste("for the data below find:
            <p>&nbsp;<p>Mean = ", nm(xbar, w=c(100, 90, 80),eps=c(0, 0.05, 0.1)),
           "<p>&nbsp;<p>Median = ", nm(med, w=c(100, 90, 80),eps=c(0,0.05, 0.1)),
           "<p>&nbsp;<p>Standard Deviation = ", nm(shat, w=c(100, 90, 80),eps=c(0,0.05, 0.1)),
           "<p>&nbsp;<p>Inter quartile range = ", nm(iqr, w=c(100, 90, 80),eps=c(0,0.05, 0.1)),
           "<p>&nbsp;<p>90<sup>th</sup> Percentile = ", nm(p90, w=c(100, 90, 8090, 80),eps=c(0,0.05, 0.1)),
           "<p>&nbsp;<p>3<sup>rd</sup> Quartile = ", nm(q3, w=c(100, 90, 80),eps=c(0,0.05, 0.1)))
        htxt <- ""   
   ...
   if(problem==2) {
        ...
        new.n <- prop.ps(E=E, phat=round(x/n, 4), conf.level=90, return.result=TRUE)
        new.n[2] <- prop.ps(E=E, phat=round(x/n, 4), return.result=TRUE)
        ...
        qtxt <- paste0("In a survey of ", n, " people ", x, " said they were planning a
            ...
            who are planning a vacation next month is (", 
              nm(c(100*ci1[1], 100*ci2[1], ci1[1], ci2[1]), w=c(100, 90, 90, 80)), ", ",
              nm(c(100*ci1[2], 100*ci2[2], ci1[2], ci2[2]), w=c(100, 90, 90, 80)), ")
            <p>&nbsp;<p>If they want to find this  interval with an error of ", E, 
            ", the sample size needed is ", nm(new.n, w=c(100, 90, 70, 60)), eps=1)
   ...
   if(problem==3) {
       ...
       pval <- one.sample.t(xbar, shat=sigma, n=n, mu.null=mu0, alternative="less", return.result=TRUE)
       pval[2] <- one.sample.t(xbar, shat=sigma, n=n, mu.null=mu0, return.result=TRUE)
       if(pval[1]<0.1) {
       ...
       qtxt <- paste0("In the past the mean number of phone calls received by a tech support  
        ...
        "<p>H<sub>a</sub>: ", mc(parlist, w=c(100, rep(0, 5))),  " ", mc(sgnlist, c(0, 100, 0)), 
                             " ", nm(mu0),                    
        "<p>p value = ", nm(pval, w=c(100, 75), pts=5),
   ...
   if(problem==5) {
         ...
         fit <- slr(df$Weight, df$Height, return.result=TRUE, return.graph=FALSE)[1:2]
         est <- round(fit[1] + fit[2]*175, 1)
         qtxt <- paste("Below are the height (in cm) and weight (in kg) of ", n, 
            " randomly selected men. 
           <p>&nbsp;<p>The correlation coefficient of height and weight is ", nm(r, ndigits=3),
          "<p>&nbsp;<p>The least squares regression equation is 
           <p>Weight = ", nm(fit[1], eps=0.9), " + ",  nm(fit[2], eps=0.1), " Height
           <p>&nbsp;<p>The  predicted weight of a man who is 175 cm tall is ", nm(est, eps=0.9), 
           "</h5><hr>", moodle.table(df))
           df<-""
         atxt <- paste("r = ", r, 
   ...
   if(problem==6) {
        ...
        qtxt <- paste0("Over many years ", old.p, "% percent of the parts made in a factory where
            ...
            <p>&nbsp;<p>n = ", nm(n, w=c(100, 80), eps=rep(2, 2)))
   ...
   if(problem==7) {
        ...
        df[df<0]=0
        n <- c(t.ps(E=3, sigma=sd(df), conf.level=90, return.result=TRUE),
             t.ps(E=3, sigma=sd(df), return.result=TRUE),
             t.ps(E=6, sigma=sd(df), conf.level=90, return.result=TRUE),
             t.ps(E=6, sigma=sd(df), return.result=TRUE))
        
        qtxt <- paste0("A researcher wants to find out how much time people spend every day on
           social media. She wants to find a 90% confidence interval for the mean. She does
           a small pilot study and the replies are below (in minutes). How many people does
           she have to include in his study if she wants the interval to have a length of 6
           minutes?
           <p>&nbsp;<p>n = ", nm(n, w=c(100, 90, 70, 60), eps=rep(2, 4)))
        atxt <- paste0("E = L/2 = 3
           <p>t.ps(E=3, sigma=sd(minutes), conf.level=90)
           <p>n = ", n[1])   
    } 
    if(problem==8) {  
        p1=sample(sequence(44, 46, 0.1)/100, 1)  
        pow1=prop.ps(1000,phat=p1, pi.null=0.5, return.result = TRUE)
        qtxt <- paste0("Say we are planning a survey of 1000 likely voters in an election with two
                candidates A and B. If in reality ", 100*p1, "% of voters will vote for A, 
                the power of the test that the two candidates are exactly tied is ", nm(pow1, eps=0.1))
        atxt <- paste0("prop.ps(1000,phat=", p1, ", pi.null=0.5) = ", pow1)                    
    }
    if(problem==9) {  
        n=sample(50:60, 1)
        n1=sample(20:30, 1)
        df=sample(c(rep("Agree ",n1), rep("Disagree ", n-n1)))  
        ci1=one.sample.prop(n1, n, conf.level=90, return.result = TRUE)
        ci2=one.sample.prop(n1, n, return.result = TRUE)
        qtxt <- paste0("In a survey participants were asked whether they agreed or disagreed
           with the statement: In general the food in the Cafeteria is ok. The answers are below.
           <p>A 90% confidence interval for the percentage of people who find the Cafeteria ok is (",
              nm(c(100*ci1[1], 100*ci2[2], ci1[1], ci2[2]), w=c(100, 90, 90, 80)), ", ",
              nm(c(100*ci1[2], 100*ci2[2], ci1[2], ci2[2]), w=c(100, 90, 90, 80)), ")")
              
        atxt <- paste0("one.sample.prop(", n1, ", ", n, ", conf.level=90): (", 100*ci1[1], ", ", 100*ci1[2], ")")                    
    }    
