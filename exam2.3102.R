...
#      out1 <- tukey(y, x, return.result = TRUE)
...
#      "<p>Result of tukey command:<<p>R commands:<p>", command1,
#        moodle.table(out1), "<p>",command2)                               
   }    
   if(problem == 2) {   
      n <- sample(30:40, 1)
      x <- 1:n
  ...
  if(problem==5) {
      ...
      atxt <- paste0("The data is not in the proper format, so we need to 
        ...
        <p>copy data with water and save it as x1: get.moodle.data();x1=x
        <p>copy data without water and save it as x2: get.moodle.data();x2=x
        <p>make vector identifying water or no water:
